...
3. Run **Puff — Design Context Adapter** from development plugins.
4. Open the engineering-only page `http://127.0.0.1:4318/?developer=1`, choose **Connect local backend**, expand connection settings and reveal the local pairing token. Paste that token into the adapter. This token is not an OpenAI/OpenRouter/Exa API key. Do not show developer setup during the demo.
...
Remote collaborator edits are ignored as activity signals. The adapter does not call a model directly. A separate approval in Puff allows the backend to send context to the configured model provider.
