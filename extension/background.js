...
chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  const perform=async()=>{
    ...
    if(message.type==="GET_PAGE"){
      // The confirm screen has to name the page before anything is saved.
      if(!isPanel(sender)||!sender.tab?.url?.startsWith("http"))return {ok:true,page:null};
      return {ok:true,page:{title:(sender.tab.title||"").slice(0,200),url:sender.tab.url,tabId:sender.tab.id}};
    }
    if(message.type==="SAVE_HOLD"){
      if(!isPanel(sender)||!sender.tab?.url?.startsWith("http"))throw new Error("Open Puff on a normal web page.");
...
chrome.alarms.onAlarm.addListener(alarm=>{if(alarm.name==="evaluate-off-ramp")serialize(async()=>saveState(OffRampCore.evaluate(await getState())));});
